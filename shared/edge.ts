...
export function renderDeepLinkEscapePage(targetUrl: string, userAgent = ""): string {
  ...
  const redditIos = isIos && /reddit/i.test(userAgent);
  ...
  return renderFastBrowserTrampoline(targetUrl, browserName, isAndroid, safeTarget, redditIos);
}

function renderFastBrowserTrampoline(targetUrl: string, browserName: string, isAndroid: boolean, safeTarget: string, redditIos: boolean): string {
  const jsTarget = JSON.stringify(targetUrl);
  const jsBrowser = JSON.stringify(browserName);
  const jsIsAndroid = JSON.stringify(isAndroid);
  const jsRedditIos = JSON.stringify(redditIos);

  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Opening link</title></head><body style="background:#fff"><script>(function(){var escapeUrl=${jsTarget};var escapeTo=${jsBrowser};var isAndroid=${jsIsAndroid};var isRedditIos=${jsRedditIos};var freshHtml='<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="background:#fff">';freshHtml+='<div id="trampoline-overlay" style="position:fixed;inset:0;z-index:99999;background:#fff"></div>';freshHtml+='<scr'+'ipt>';freshHtml+='(function(){';freshHtml+='var escapeUrl='+JSON.stringify(escapeUrl)+';';freshHtml+='var escapeTo='+JSON.stringify(escapeTo)+';';freshHtml+='var isAndroid='+JSON.stringify(isAndroid)+';';freshHtml+='var isRedditIos='+JSON.stringify(isRedditIos)+';';freshHtml+='function fresh(url){return url+(url.indexOf("?")!==-1?"&":"?")+"_t="+Date.now()+Math.random().toString(36).substring(2,6);}';freshHtml+='function openBlank(url){try{var a=document.createElement("a");a.href=url;a.target="_blank";a.rel="noreferrer noopener";document.body.appendChild(a);a.click();document.body.removeChild(a);}catch(e){window.open(url,"_blank");}}';freshHtml+='var uniqueUrl=fresh(escapeUrl);';freshHtml+='if(isAndroid){try{var u=new URL(uniqueUrl);window.location.href="intent://"+u.host+u.pathname+u.search+u.hash+"#Intent;scheme="+u.protocol.replace(":","")+";package=com.android.chrome;S.browser_fallback_url="+encodeURIComponent(uniqueUrl)+";end";}catch(e){window.location.href=uniqueUrl;}}else if(isRedditIos){openBlank(uniqueUrl);setTimeout(function(){window.location.href="com-apple-mobilesafari-tab:"+uniqueUrl;},120);}else{var w=uniqueUrl.replace(/^https?:\\\\/\\\\//,"");window.location.href="x-safari-https://"+w;setTimeout(function(){window.location.href="com-apple-mobilesafari-tab:"+uniqueUrl;},200);}';freshHtml+='setTimeout(function(){var o=document.getElementById("trampoline-overlay");if(!o)return;var w2=document.createElement("div");w2.style.cssText="position:absolute;bottom:60px;left:0;right:0;text-align:center";var b=document.createElement("a");b.href="#";b.style.cssText="display:inline-block;padding:14px 32px;background:#000;color:#fff;font-weight:600;font-size:16px;border-radius:12px;text-decoration:none;font-family:-apple-system,BlinkMacSystemFont,sans-serif";b.textContent="Open in "+escapeTo;b.onclick=function(ev){ev.preventDefault();var u2=fresh(escapeUrl);if(isAndroid){try{var u3=new URL(u2);window.location.href="intent://"+u3.host+u3.pathname+u3.search+u3.hash+"#Intent;scheme="+u3.protocol.replace(":","")+";package=com.android.chrome;end";}catch(e2){window.location.href=u2;}}else if(isRedditIos){openBlank(u2);setTimeout(function(){window.location.href="com-apple-mobilesafari-tab:"+u2;},120);setTimeout(function(){window.location.href="x-safari-https://"+u2.replace(/^https?:\\\\/\\\\//,"");},500);}else{var rn=Math.random().toString(36).substring(2,10);window.location.href="shortcuts://x-callback-url/run-shortcut?name="+rn+"&x-error="+encodeURIComponent(u2);setTimeout(function(){window.location.href="x-safari-https://"+u2.replace(/^https?:\\\\/\\\\//,"");},100);setTimeout(function(){window.location.href="com-apple-mobilesafari-tab:"+u2;},250);}setTimeout(function(){window.open(u2,"_blank");},2000);};w2.appendChild(b);o.appendChild(w2);},1500);';freshHtml+='})();';freshHtml+='</scr'+'ipt></body></html>';document.open();document.write(freshHtml);document.close();})();</script><noscript><p><a href="${safeTarget}">Open in ${escapeHtml(browserName)}</a></p></noscript></body></html>`;
}
